...
async function create(page,label,values){await page.getByRole('button',{name:'+ 新建'}).click();await page.locator('#picker').getByRole('button',{name:label,exact:true}).click();for(const [name,value] of Object.entries(values))await page.locator('#editor').getByLabel(name,{exact:true}).fill(value);await page.locator('#editor').getByRole('button',{name:'保存'}).click();}
