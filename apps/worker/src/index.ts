...
export default {async fetch(req:Request,env:Env):Promise<Response>{
 try{
  ...
  const am=p.match(/^\/api\/attachments\/([^/]+)(?:\/(content|metadata))?$/);if(am){const id=decodeURIComponent(am[1]);if(!validAttachmentId(id))return error(400,'invalid_id');const action=am[2];
   if(req.method==='POST'&&!action){const declared=req.headers.get('content-length');if(declared===null)return error(411,'length_required');const length=Number(declared);if(!Number.isSafeInteger(length)||length<16||length>MAX_WORKER_UPLOAD||!req.body)return error(413,'too_large');let metadata:unknown;try{metadata=JSON.parse(req.headers.get('x-attachment-metadata')||'')}catch{return error(400,'invalid_metadata')}if(!validAttachmentEnvelope(metadata))return error(400,'invalid_metadata');if(await env.DB.prepare('SELECT 1 FROM attachments WHERE user_id=? AND id=?').bind(s.user_id,id).first())return error(409,'attachment_exists');const bytes=await req.arrayBuffer();if(bytes.byteLength!==length)return error(400,'invalid_content_length');await reserveStorage(env,length);await reserveOperation(env,'class_a');const key=`${await digest(s.user_id)}/${random(32)}`;try{await env.ATTACHMENTS.put(key,bytes,{httpMetadata:{contentType:'application/octet-stream'}})}catch(e){await env.ATTACHMENTS.delete(key);throw e}const now=Date.now();try{await env.DB.prepare('INSERT INTO attachments VALUES(?,?,?,?,?,?,?,?)').bind(s.user_id,id,(metadata as any).iv,(metadata as any).ciphertext,key,length,now,now).run()}catch(e){await env.ATTACHMENTS.delete(key);throw e}return json({id,metadata,ciphertextSize:length,createdAt:now,updatedAt:now},201)}
  ...
  if(req.method==='PUT'&&p==='/api/backup'){
   ...
   const old=(await env.DB.prepare('SELECT id,metadata_iv,metadata_ciphertext,object_key,ciphertext_size,created_at,updated_at FROM attachments WHERE user_id=? ORDER BY updated_at DESC').bind(s.user_id).all()).results as any[],oldTotal=(old as any[]).reduce((n,row)=>n+Number(row.ciphertext_size),0),uploaded:string[]=[];await reserveStorage(env,total-oldTotal);try{for(const a of prepared){await reserveOperation(env,'class_a');await env.ATTACHMENTS.put(a.key,a.bytes,{httpMetadata:{contentType:'application/octet-stream'}});uploaded.push(a.key)}const now=Date.now();await env.DB.batch([env.DB.prepare('UPDATE users SET kdf=?,wrapped_key=? WHERE id=?').bind(JSON.stringify(x.kdf),JSON.stringify(x.wrappedKey),s.user_id),env.DB.prepare('DELETE FROM entries WHERE user_id = ?').bind(s.user_id),env.DB.prepare('DELETE FROM attachments WHERE user_id = ?').bind(s.user_id),...entries.map((v:any)=>env.DB.prepare('INSERT INTO entries(user_id,id,type,version,iv,ciphertext,updated_at) VALUES(?,?,?,?,?,?,?)').bind(s.user_id,v.id,v.type,v.version,v.iv,v.ciphertext,now)),...prepared.map(a=>env.DB.prepare('INSERT INTO attachments VALUES(?,?,?,?,?,?,?,?)').bind(s.user_id,a.id,a.metadata.iv,a.metadata.ciphertext,a.key,a.ciphertextSize,a.createdAt||now,a.updatedAt||now))])}catch(e){for(const key of uploaded)await env.ATTACHMENTS.delete(key);throw e}for(const row of old){try{await env.ATTACHMENTS.delete(row.object_key)}catch(e){console.error(JSON.stringify({event:'attachment_cleanup_failed'}))}}return json({ok:true,count:entries.length,attachments:prepared.length})
  }
