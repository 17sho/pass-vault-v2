...
export default {async fetch(req:Request,env:Env):Promise<Response>{
 try{
  ...
  const am=p.match(/^\/api\/attachments\/([^/]+)(?:\/(content|metadata))?$/);if(am){const id=decodeURIComponent(am[1]);if(!validAttachmentId(id))return error(400,'invalid_id');const action=am[2];
   if(req.method==='POST'&&!action){const declared=req.headers.get('content-length');if(declared===null)return error(411,'length_required');const length=Number(declared);if(!Number.isSafeInteger(length)||length<16||length>MAX_WORKER_UPLOAD||!req.body)return error(413,'too_large');let metadata:unknown;try{metadata=JSON.parse(req.headers.get('x-attachment-metadata')||'')}catch{return error(400,'invalid_metadata')}if(!validAttachmentEnvelope(metadata))return error(400,'invalid_metadata');if(await env.DB.prepare('SELECT 1 FROM attachments WHERE user_id=? AND id=?').bind(s.user_id,id).first())return error(409,'attachment_exists');const bytes=await req.arrayBuffer();if(bytes.byteLength!==length)return error(400,'invalid_content_length');await reserveStorage(env,length);let committed=false,key='';const now=Date.now();try{await reserveOperation(env,'class_a');key=`${await digest(s.user_id)}/${random(32)}`;await env.ATTACHMENTS.put(key,bytes,{httpMetadata:{contentType:'application/octet-stream'}});await env.DB.prepare('INSERT INTO attachments VALUES(?,?,?,?,?,?,?,?)').bind(s.user_id,id,(metadata as any).iv,(metadata as any).ciphertext,key,length,now,now).run();committed=true;return json({id,metadata,ciphertextSize:length,createdAt:now,updatedAt:now},201)}catch(e){if(key)try{await env.ATTACHMENTS.delete(key)}catch{}throw e}finally{if(!committed)await reserveStorage(env,-length)}}
   ...
   if(req.method==='DELETE'&&!action){await env.ATTACHMENTS.delete(row.object_key);await env.DB.prepare('DELETE FROM attachments WHERE user_id=? AND id=?').bind(s.user_id,id).run();await reserveStorage(env,-Number(row.ciphertext_size));return new Response(null,{status:204,headers:{...SECURITY_HEADERS,'cache-control':'no-store'}})}
  }
  ...
  if(req.method==='PUT'&&p==='/api/backup'){
   ...
   if(x.version===1){if(!validKey(x)||!Array.isArray(x.envelopes)||x.envelopes.length>10000)return error(400,'invalid_backup');const items=x.envelopes.map((v:any)=>envelope(v));if(items.some((v:any)=>!v)||new Set(items.map((v:any)=>v.id)).size!==items.length)return error(400,'invalid_backup');const now=Date.now();await env.DB.batch([env.DB.prepare('UPDATE users SET kdf=?,wrapped_key=? WHERE id=?').bind(JSON.stringify(x.kdf),JSON.stringify(x.wrappedKey),s.user_id),env.DB.prepare('DELETE FROM entries WHERE user_id = ?').bind(s.user_id),...items.map((v:any)=>env.DB.prepare('INSERT INTO entries(user_id,id,type,version,iv,ciphertext,updated_at) VALUES(?,?,?,?,?,?,?)').bind(s.user_id,v.id,v.type,v.version,v.iv,v.ciphertext,now))]);return json({ok:true,count:items.length,attachments:0})}
   ...
   const old=(await env.DB.prepare('SELECT id,metadata_iv,metadata_ciphertext,object_key,ciphertext_size,created_at,updated_at FROM attachments WHERE user_id=? ORDER BY updated_at DESC').bind(s.user_id).all()).results as any[],oldTotal=(old as any[]).reduce((n,row)=>n+Number(row.ciphertext_size),0),uploaded:string[]=[],delta=total-oldTotal;await reserveStorage(env,delta);let committed=false;try{for(const a of prepared){await reserveOperation(env,'class_a');await env.ATTACHMENTS.put(a.key,a.bytes,{httpMetadata:{contentType:'application/octet-stream'}});uploaded.push(a.key)}const now=Date.now();await env.DB.batch([env.DB.prepare('UPDATE users SET kdf=?,wrapped_key=? WHERE id=?').bind(JSON.stringify(x.kdf),JSON.stringify(x.wrappedKey),s.user_id),env.DB.prepare('DELETE FROM entries WHERE user_id = ?').bind(s.user_id),env.DB.prepare('DELETE FROM attachments WHERE user_id = ?').bind(s.user_id),...entries.map((v:any)=>env.DB.prepare('INSERT INTO entries(user_id,id,type,version,iv,ciphertext,updated_at) VALUES(?,?,?,?,?,?,?)').bind(s.user_id,v.id,v.type,v.version,v.iv,v.ciphertext,now)),...prepared.map(a=>env.DB.prepare('INSERT INTO attachments VALUES(?,?,?,?,?,?,?,?)').bind(s.user_id,a.id,a.metadata.iv,a.metadata.ciphertext,a.key,a.ciphertextSize,a.createdAt||now,a.updatedAt||now))]);committed=true}catch(e){for(const key of uploaded)try{await env.ATTACHMENTS.delete(key)}catch{}throw e}finally{if(!committed)await reserveStorage(env,-delta)}for(const row of old){try{await env.ATTACHMENTS.delete(row.object_key)}catch(e){console.error(JSON.stringify({event:'attachment_cleanup_failed'}))}}return json({ok:true,count:entries.length,attachments:prepared.length})
  }
