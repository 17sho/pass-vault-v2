...
export default {async fetch(req:Request,env:Env):Promise<Response>{
 try{
  ...
  const am=p.match(/^\/api\/attachments\/([^/]+)(?:\/(content|metadata))?$/);if(am){const id=decodeURIComponent(am[1]);if(!validAttachmentId(id))return error(400,'invalid_id');const action=am[2];
   if(req.method==='POST'&&!action){const declared=req.headers.get('content-length'),length=declared===null?null:Number(declared);if((length!==null&&(!Number.isSafeInteger(length)||length<16||length>MAX_WORKER_UPLOAD))||!req.body)return error(413,'too_large');let metadata:unknown;try{metadata=JSON.parse(req.headers.get('x-attachment-metadata')||'')}catch{return error(400,'invalid_metadata')}if(!validAttachmentEnvelope(metadata))return error(400,'invalid_metadata');if(await env.DB.prepare('SELECT 1 FROM attachments WHERE user_id=? AND id=?').bind(s.user_id,id).first())return error(409,'attachment_exists');let actual=0;const counted=req.body.pipeThrough(new TransformStream({transform(chunk,controller){actual+=chunk.byteLength;if(actual>MAX_WORKER_UPLOAD)throw new RangeError();controller.enqueue(chunk)}}));const key=`${await digest(s.user_id)}/${random(32)}`;try{await env.ATTACHMENTS.put(key,counted,{httpMetadata:{contentType:'application/octet-stream'}})}catch(e){await env.ATTACHMENTS.delete(key);throw e}if(actual<16){await env.ATTACHMENTS.delete(key);return error(413,'too_large')}if(length!==null&&actual!==length){await env.ATTACHMENTS.delete(key);return error(400,'invalid_content_length')}const now=Date.now();try{await env.DB.prepare('INSERT INTO attachments VALUES(?,?,?,?,?,?,?,?)').bind(s.user_id,id,(metadata as any).iv,(metadata as any).ciphertext,key,actual,now,now).run()}catch(e){await env.ATTACHMENTS.delete(key);throw e}return json({id,metadata,ciphertextSize:actual,createdAt:now,updatedAt:now},201)}
   ...
   if(req.method==='GET'&&action==='content'){const object=await env.ATTACHMENTS.get(row.object_key);if(!object)return error(404,'not_found');return new Response(object.body,{headers:{...SECURITY_HEADERS,'content-type':'application/octet-stream','content-length':String(row.ciphertext_size),'cache-control':'no-store'}})}
   ...
   if(req.method==='DELETE'&&!action){await env.ATTACHMENTS.delete(row.object_key);await env.DB.prepare('DELETE FROM attachments WHERE user_id=? AND id=?').bind(s.user_id,id).run();return new Response(null,{status:204,headers:{...SECURITY_HEADERS,'cache-control':'no-store'}})}
  }
  ...
  if(req.method==='GET'&&p==='/api/backup'){
   ...
   for(const row of ar.results as any[]){const object=await env.ATTACHMENTS.get(row.object_key);if(!object)return error(409,'missing_attachment_object');const bytes=await new Response(object.body).arrayBuffer();if(bytes.byteLength!==row.ciphertext_size)return error(409,'attachment_size_mismatch');attachments.push({id:row.id,metadata:{version:1,iv:row.metadata_iv,ciphertext:row.metadata_ciphertext},ciphertextSize:row.ciphertext_size,createdAt:row.created_at,updatedAt:row.updated_at,sha256:await digestBytes(bytes),object:bytesToB64(bytes)})}
