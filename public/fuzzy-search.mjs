export const searchableFields={
 account:['platform','loginUrl','username','notes','tags'],
...
export function rankSearchResults(items,query,fields){
 ...
 return items.map((item,index)=>({item,index,score:Math.min(...fields.map(field=>fieldScore(normalized,item[field])))})).filter(x=>Number.isFinite(x.score)).sort((a,b)=>a.score-b.score||a.index-b.index).map(x=>x.item);
}
