...
### 从 v1.1.16 或更早版本升级到 v1.1.19

v1.1.17–v1.1.19 只增加修改登录名及相关前端布局/焦点修复，**没有新增 D1 migration、R2 变更、binding、环境变量或 Secret**。现有 `INVITE_CODE`、`DB` 和 `ATTACHMENTS` 配置保持不变；不需要清空、导入或重建数据库，也不需要重新加密现有密码库。仍应先按下述流程备份 D1 + R2、运行现有 migration 命令确认无待应用迁移，再构建并部署。若从更早版本升级，必须按文件名顺序应用仓库中全部尚未执行的 migration，不能把本说明理解为可以跳过旧迁移。

部署后确认首页引用 `app.mjs?v=1.1.19`，再验证既有用户登录、修改登录名（成功后全部会话退出）以及分组弹窗聚焦当前分组。
