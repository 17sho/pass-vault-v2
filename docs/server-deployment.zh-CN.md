...
v1.1.17–v1.1.19 只增加修改登录名及相关前端布局/焦点修复，**没有新增 SQLite migration、附件存储变更、环境变量、Secret 或 systemd 配置**。现有 `INVITE_CODE` 和数据目录保持不变；不需要清空、导入或重建 SQLite，也不需要重新加密现有密码库。Linux 服务启动时仍会执行既有幂等 schema 初始化；若从更早版本升级，必须保留并执行该初始化，不能跳过旧 schema 变更。

切换版本并重启后，确认首页引用 `app.mjs?v=1.1.19`，再验证既有用户登录、修改登录名（成功后全部会话退出）以及分组弹窗聚焦当前分组。
