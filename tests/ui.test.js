...
test('新建类型弹窗打开时标题获得初始焦点，关闭叉不显示焦点框',async()=>{
 const page=await browser.newPage({viewport:{width:390,height:844}});await register(page);await page.getByRole('button',{name:'+ 新建'}).click();const picker=page.locator('#picker'),close=picker.getByRole('button',{name:'关闭'});assert.equal(await picker.evaluate(e=>e.contains(document.activeElement)&&document.activeElement===e.querySelector('h2')),true);assert.equal(await close.evaluate(e=>e.matches(':focus-visible')),false);await page.keyboard.press('Tab');assert.equal(await picker.getByRole('button',{name:'账号',exact:true}).evaluate(e=>e.matches(':focus-visible')),true);await page.keyboard.press('Escape');await picker.waitFor({state:'hidden'});assert.equal(await page.evaluate(()=>document.activeElement?.id),'add');await page.close();
});
...
test('附件库上传筛选预览改名删除，笔记可关联与移除图片',async()=>{
 ...
 await page.locator('#detail').getByRole('button',{name:'重命名'}).click();const rename=page.getByRole('dialog',{name:'重命名附件'});await rename.getByLabel('文件名').fill('renamed.png');await rename.getByRole('button',{name:'保存'}).click();await page.getByText('renamed.png',{exact:true}).waitFor();await page.locator('#detail').getByRole('button',{name:'← 返回'}).click();assert.ok((await page.evaluate(()=>window.__revoked.length))>0);
